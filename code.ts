...
figma.ui.onmessage = async (msg: { type: string; [key: string]: any }) => {
  ...
  if (msg.type === 'create-guidance') {
    try {
      ...
      figma.ui.postMessage({ type: 'done-guidance', ...result });
    } catch (e: any) {
      figma.ui.postMessage({ type: 'error', message: String(e?.message ?? e) });
    }
  }
...
type TokenDef = { $value: string | number; $type: string };
type TokenGroup = Record<string, TokenDef>;

type CollectionGroup = {
  collectionName: string;
  modes: { key: string; modeName: string }[];
};
...
function hexToRgba(hex: string): RGBA {
  const h = hex.replace('#', '');
  const p = (s: number) => parseInt(h.slice(s, s + 2), 16) / 255;
...
function parseRef(v: string): { group: string; name: string } | null {
 ...
 */
function groupSets(order: string[]): CollectionGroup[] {
  ...
  for (const key of order) {
    ...
    if (key.includes('/')) {
      ...
      if (!map.has(cn)) map.set(cn, { collectionName: cn, modes: [] });
...
async function runImport(data: Record<string, any>): Promise<ImportResult> {
  ...
  for (const group of groups) {
    ...
    // ── Set up modes ──────────────────────────────────────────────────────────
    const modeIds = new Map<string, string>(); // setKey → modeId
    ...
    for (const { key } of group.modes) {
      Object.keys((data[key] as TokenGroup) ?? {}).forEach((n) => allNames.add(n));
    }
    ...
    for (const name of allNames) {
      ...
      for (const { key } of group.modes) {
        const def = (data[key] as TokenGroup)?.[name];
        if (def) {
    ...
    for (const name of allNames) {
      ...
      for (const { key } of group.modes) {
        ...
        const def = (data[key] as TokenGroup)?.[name];
        if (!def) continue; // this mode doesn't define this token — leave Figma's default
        ...
        try {
          ...
          if (ref) {
            ...
            const target =
              varMap.get(`${ref.group}/${ref.name}`) ??
              allExistingVars.find((v) => v.name === ref.name);
...
type GuidanceResult = { sections: number; nodes: number };

type PrimitiveDef = { $value: any; $type: string };
type SemanticDef = { $value: string; $type: string };
...
async function runCreateGuidance(data: Record<string, any>): Promise<GuidanceResult> {
  ...
  let page = figma.root.children.find((p: PageNode) => p.name === '🎨 Token Guidance');
  if (!page) {
  ...
  const C = {
    brand:    { r: 0.486, g: 0.231, b: 0.929 } as RGB,
    gray900:  { r: 0.059, g: 0.059, b: 0.071 } as RGB,
    gray600:  { r: 0.275, g: 0.275, b: 0.310 } as RGB,
    gray400:  { r: 0.580, g: 0.580, b: 0.620 } as RGB,
    gray200:  { r: 0.886, g: 0.886, b: 0.902 } as RGB,
    gray100:  { r: 0.941, g: 0.941, b: 0.949 } as RGB,
  ...
  function autoFrame(
    ...
    gap: number = 0,
    pad: [number, number, number, number] = [0, 0, 0, 0],
  ): FrameNode {
  ...
  function placeSection(frame: FrameNode): void {
  ...
  const prim     = (data['primitives']     ?? {}) as Record<string, PrimitiveDef>;
  const semLight = (data['semantic/light'] ?? {}) as Record<string, SemanticDef>;
  const semDark  = (data['semantic/dark']  ?? {}) as Record<string, SemanticDef>;
  const typoSet  = (data['typography']     ?? {}) as Record<string, SemanticDef>;
  const spacingSet = (data['spacing']      ?? {}) as Record<string, SemanticDef>;
  const radiiSet   = (data['radii']        ?? {}) as Record<string, SemanticDef>;

  function resolveRef(ref: string): PrimitiveDef | null {
    const m = String(ref).match(/^\{primitives\.([\w-]+)\}$/);
    return m ? (prim[m[1]] ?? null) : null;
  }

  // ── Section 1: Color Palettes ──────────────────────────────────────────────

  const colorScales = new Map<string, Array<{ step: string; hex: string }>>();
  for (const [name, def] of Object.entries(prim)) {
    if (def.$type !== 'color') continue;
    const m = name.match(/^color-(\w+)-(\d+)$/);
    if (!m) continue;
    if (!colorScales.has(m[1])) colorScales.set(m[1], []);
  ...
  if (colorScales.size > 0) {
    ...
    for (const [scaleName, steps] of colorScales) {
      ...
      for (const { step, hex } of sorted) {
        ...
        rect.resize(52, 40);
        rect.cornerRadius = 6;
        try { rect.fills = [{ type: 'SOLID', color: hexToFigmaRgb(hex) }]; }
        catch { rect.fills = [{ type: 'SOLID', color: C.gray200 }]; }
        nodes++;
  ...
  if (Object.keys(semLight).length > 0) {
    ...
    const modes: [string, Record<string, SemanticDef>][] = [
      ['Light', semLight],
      ['Dark',  semDark],
    ];
    ...
    for (const [modeName, modeTokens] of modes) {
      ...
      for (const [tokenName, def] of Object.entries(modeTokens)) {
        ...
        try { swatch.fills = [{ type: 'SOLID', color: hexToFigmaRgb(resolved?.$value ?? '') }]; }
        catch { swatch.fills = [{ type: 'SOLID', color: C.gray200 }]; }
        nodes++;
  ...
  if (styleNames.size > 0) {
    ...
    const styleList = [...styleNames].map(styleName => {
      const sizeDef   = resolveRef(typoSet[`typography-${styleName}-size`]?.$value ?? '');
      const weightDef = resolveRef(typoSet[`typography-${styleName}-weight`]?.$value ?? '');
      const lhDef     = resolveRef(typoSet[`typography-${styleName}-lineHeight`]?.$value ?? '');
    ...
    for (let i = 0; i < styleList.length; i++) {
      ...
      meta.appendChild(txt(
        `${style.sizeLabel}  ·  w${style.weightLabel}  ·  lh ${style.lhLabel}`,
        10, 'Regular', C.gray400,
      ));
      nodes++;
      ...
      if (i < styleList.length - 1) list.appendChild(divider());
    }
  ...
  const primSpacing: Array<{ name: string; px: number }> = [];
  for (const [name, def] of Object.entries(prim)) {
    if (def.$type !== 'dimension' || !name.startsWith('spacing-')) continue;
  ...
  const primRadii: Array<{ name: string; value: string; px: number }> = [];
  for (const [name, def] of Object.entries(prim)) {
    if (def.$type !== 'dimension' || !name.startsWith('radius-')) continue;
    ...
    if (hasSpacing) {
      ...
      for (const [tokenName, def] of Object.entries(spacingSet)) {
        ...
        info.appendChild(txt(
          resolvedPx !== null ? `${def.$value}  (${resolvedPx}px)` : def.$value,
          9, 'Regular', C.gray400,
        ));
        row.appendChild(info);
        col.appendChild(row);
